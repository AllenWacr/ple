    ...
    <OrganizationLayout title="Dashboard">
                ...
                <div v-for="courseModule in course.modules">
                    <a-typography-title :level="3">
                        <span v-if="courseModule.edit">
                            <input v-model="courseModule.label" :readonly="false"
                                @keyup.enter="onModuleLabelUpdate(courseModule)" />
                        </span>
                        <span v-else>
                            {{ courseModule.label }}
                            <span role="img" aria-label="form" class="anticon anticon-form ant-menu-item-icon"
                                @click="courseModule.edit = true">
                                <svg focusable="false" class="" data-icon="form" width="1em" height="1em"
                                    ...
                                    <tr v-for="content in course.contents">
                                        <td v-for="column in columns">
                                            <div v-if="column.dataIndex == 'operation'">
                                                ...
                                                <a-button @click="deleteRecord(content.course_id, content.id)"
        ...
        <a-modal v-model:visible="deleteModal.isOpen" :title="$t(deleteModal.title)" width="60%"
            ...
            <a-form ref="modalRef" :model="deleteModal.data" name="Certificate" :label-col="{ span: 8 }"
                :wrapper-col="{ span: 16 }" autocomplete="off" enctype="multipart/form-data">
